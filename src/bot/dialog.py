...
def hello(user_id):
    ...
    yes = yield from ask_yes_or_no("Приятно познакомиться, %s.\nВы работаете на АЗК/АЗС?\n" % name)

    if yes:
        answer = yield "Напишите номер АЗК/АЗС.\n" \
                       "Например: 55\n"
    else:
        answer = yield "Напишите компанию и должность\n" \
                       "Например: Иркутскнефтепродукт, территориальный менеджер.\n"
